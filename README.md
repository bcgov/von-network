...
- `LEDGER_CACHE_PATH`: if set, it will instruct the ledger to create an on-disk cache, rather than in-memory. The specified file path needs to be in a location that can vbe written by the `indy` user (e.g.: `/home/indy/.indy-client/ledger_cache_file).
